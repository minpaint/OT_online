...
class CommissionAdmin(CommissionTreeViewMixin, admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        """
        Переопределяем метод для фильтрации комиссий по профилю пользователя.
        Обычные пользователи видят только комиссии своих организаций.
        """
        ...
        if not request.user.is_superuser and hasattr(request.user, 'profile'):
            ...
            # Фильтруем комиссии на всех уровнях (организация/подразделение/отдел)
            qs = qs.filter(
                # Комиссии на уровне организации
                organization__in=allowed_orgs
            ) | qs.filter(
                # Комиссии на уровне подразделения
                subdivision__organization__in=allowed_orgs
            ) | qs.filter(
                # Комиссии на уровне отдела
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        # Ограничиваем выбор организаций организациями из профиля пользователя
        if not request.user.is_superuser and hasattr(request.user, 'profile'):
            allowed_orgs = request.user.profile.organizations.all()
...
class CommissionMemberAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        """
        Переопределяем метод для фильтрации участников комиссий по профилю пользователя.
        """
        qs = super().get_queryset(request)
        ...
        if not request.user.is_superuser and hasattr(request.user, 'profile'):
            allowed_orgs = request.user.profile.organizations.all()
            # Фильтруем участников по комиссиям, относящимся к доступным организациям
            qs = qs.filter(
                # Комиссии на уровне организации
                commission__organization__in=allowed_orgs
            ) | qs.filter(
                # Комиссии на уровне подразделения
                commission__subdivision__organization__in=allowed_orgs
            ) | qs.filter(
                # Комиссии на уровне отдела
                commission__department__organization__in=allowed_orgs
            )
        return qs
