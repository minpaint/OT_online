# directory/admin/mixins/commission_tree_view.py
    ...
    def _add_members_to_item(self, item):
        ...
        if hasattr(obj, 'members'):
            ...
            for member in members:
                # Исправляем получение должности: проверяем различные варианты полей,
                # где может храниться должность сотрудника
                position = ''
                if hasattr(member.employee, 'position'):
                ...
                roles[member.role].append({
                    ...
                    'position': position,  # Используем найденную должность
