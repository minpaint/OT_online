...
class Command(BaseCommand):
    ...
    def _create_all_orders_template(self, templates_dir):
        """
        Создает шаблон распоряжений о стажировке (объединенный)
        """
        ...
        template_path = os.path.join(templates_dir, 'all_orders_template.docx')
        ...
        if not os.path.exists(template_path):
            ...
            placeholder_content = (
                "Шаблон документа: Распоряжения о стажировке (объединенный шаблон)\n\n"
            ...
            template.template_file.save('all_orders_template.docx', file_content)
    ...
    def _create_siz_card_template(self, templates_dir):
        ...
        # Путь к файлу шаблона
        template_path = os.path.join(templates_dir, 'siz_card_template.docx')
        ...
        if not os.path.exists(template_path):
            ...
            # Создаем временный файл
            file_content = ContentFile(placeholder_content.encode('utf-8'))
            template.template_file.save('siz_card_template.docx', file_content)
            ...
            self.stdout.write(self.style.SUCCESS(
                f'Создан шаблон карточки учета СИЗ из файла {template_path}'
            ))